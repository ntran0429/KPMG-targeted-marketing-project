...
```
Deceased customers serve no purpose to analysis because the new targeted customers are obviously alive.
Thus, the deceased customers will be removed.
...
```
Seems to be some corrupted text metadata.
We will remove this column also.
...
Transactions <- Transactions |> 
  mutate(
    ...
    product_first_sold_date = as.Date(as.POSIXct("1899-12-30")) + 
      lubridate::days(product_first_sold_date),
...
brand, product_size, product_size, product_line, product_class, standard_cost
are missing with the same amount of values.
Potentially, 197 transaction observations are missing this product info.
