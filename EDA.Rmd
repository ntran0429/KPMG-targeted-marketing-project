...
```{r}
# calculate the sum of Monetary by age group and wealth segment
# CD_CA_rfm |>
...
CD_CA_rfm |>
  summarise(Monetary = sum(Monetary),
            .by = c(age_group, wealth_segment)) |>
  arrange(desc(Monetary))
```

```{r}
# identify customer ids in Transaction dataset that are not in CD_CA_rfm
# these are the customers who have not made any purchases in the past year
Transactions |>
  mutate(customer_id = as.character(customer_id)) |>
  anti_join(CD_CA_rfm, by = 'customer_id') |>
  select(customer_id) |> 
  distinct()
...
```{r}
# select customer with first name Jillie
CD_CA_rfm |> filter(customer_name == 'Jillie Fyndon')
